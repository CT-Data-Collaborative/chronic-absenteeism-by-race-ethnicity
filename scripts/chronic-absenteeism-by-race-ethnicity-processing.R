...
for (i in 1:length(chronic_absent_dist_noTrend)) {
  ...
  colnames(current_file) = current_file[which( current_file$V3=='Race/Ethnicity'), ]
...
backfill_years <- expand.grid(
  ...
  `Race/Ethnicity` = unique(chronic_absent_dist$`Race/Ethnicity`),
  `Year` = years 
)
...
complete_chronic_absent <- merge(chronic_absent_fips, backfill_years, by = c("FixedDistrict", "Race/Ethnicity", "Year"), all=T)

complete_chronic_absent <- complete_chronic_absent[order(complete_chronic_absent$FixedDistrict, complete_chronic_absent$Year, complete_chronic_absent$`Race/Ethnicity`),]
...
test <- complete_chronic_absent_long[,c("District", "Year", "Race/Ethnicity")]
test2<-test[duplicated(test), ]

#Write CSV
write.table(
  complete_chronic_absent_long,
  file.path(path_to_top_level, "data", "chronic_absenteeism_by_race_ethnicity_2012-2020.csv"),
  sep = ",",
  row.names = F
)